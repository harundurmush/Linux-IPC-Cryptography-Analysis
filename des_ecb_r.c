...
int main(){
    ...
    const char *filename_1 = "tables/td_rsakey_dec_des_ecb_reader.csv";
    const char *filename_2 = "tables/td_dec_des_ecb_reader.csv";
    const char *filename_3 = "tables/td_des_ecb_reader.csv";
    ...
    if (len > 0){
        ...
        //
        // For DES (in ECB mode)
        if (!EVP_DecryptInit_ex(ctx_des, EVP_des_ecb(), NULL, decrypted_key, decrypted_iv)) {
            ERR_print_errors_fp(stderr);
        }
        //
